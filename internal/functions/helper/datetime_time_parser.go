...
func (i *FormatTimeInfo) Available(typ TimeFormatType) bool {
	return slices.Contains(i.AvailableTypes, typ)
}
...
var formatPatternMap = map[rune]*FormatTimeInfo{
	...
	'u': {
		...
		Parse:  isoWeekdayParser,
		Format: isoWeekdayFormatter,
	},
	'V': {
		AvailableTypes: []TimeFormatType{
			FormatTypeDate, FormatTypeDatetime, FormatTypeTimestamp,
...
// isoWeekdayFormatter formats Monday=1..Sunday=7, the BigQuery `%u`
// element. time.Weekday encodes Sunday=0, so map Sun -> 7 and shift.
func isoWeekdayFormatter(t *time.Time) ([]rune, error) {
	wd := int(t.Weekday())
	if wd == 0 {
		wd = 7
	}
	return []rune(fmt.Sprint(wd)), nil
}

func isoWeekdayParser(text []rune, t *time.Time) (int, error) {
	return 0, fmt.Errorf("unimplemented iso weekday matcher")
}
